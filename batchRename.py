...
def doSomeTrick(filename):
	...
	if f is not None:
		content = f.read()
		result = applyTrick(content)
		f.close()
	else:
		result = 'can\'t open '+filename+'!'
	return result

def applyTrick(content):
...
def doStuffDirect():
	...
	answer=raw_input('Do you know what you are doing!!(y)')
	if answer == 'y':
		filenames = []
		for (dirpath, dirnames, filenames) in os.walk(tTarget):
			for filename in filenames:
				if filename == 'gitignore':
					...
					os.rename(dirpath+'/'+filename,dirpath+'/.gitignore')
	else:
		print GOOD

def setup_logging(level):
	global DB
	DB = logging.getLogger('batchRename') #replace
	DB.setLevel(level)
...
def verify():
	...
	global args
	global UIflag
	parser = argparse.ArgumentParser(description='A batchRename Utility') #replace
	parser.add_argument('-v', '--verbose', dest='verbose', action = 'store_true', default=False, help='Verbose mode')
	parser.add_argument('query', nargs='*', default=None)
	parser.add_argument('-d', '--database', dest='database', action = 'store', default='/.hmDict/batchRename.db') #replace
