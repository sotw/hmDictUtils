		...
		for (dirpath, dirnames, filenames) in os.walk(tTarget):
			if len(dirnames) != 0:
				...
				for dirname in dirnames:
					print 'dealing with dirname:'+dirname
...
def setup_logging(level):
	global DB
	DB = logging.getLogger('treeMurder') #replace
	DB.setLevel(level)
...
def verify():
	...
	global args
	global UIflag
	parser = argparse.ArgumentParser(description='A treeMurder Utility') #replace
	parser.add_argument('-v', '--verbose', dest='verbose', action = 'store_true', default=False, help='Verbose mode')
	parser.add_argument('query', nargs='*', default=None)
	parser.add_argument('-d', '--database', dest='database', action = 'store', default='/.hmDict/treeMurder.db') #replace
