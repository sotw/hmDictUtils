...
def getReleaseNoteDetail(tDetail):
	...
	#videoResult = tree.xpath('//ol[@id="search-results"]//div[@class="yt-lockup-thumbnail"]//a')
	videoResult = tree.xpath('//ol')
	
	print len(videoResult)
	raw_input()

	for video in videoResult:
		if video.get('href') is not None:
...
def doStuff(tTarget):
	...
	#print result
	#print paintRED(result,'<td align="left" class="bk11">')
	global LINKS
    #head line#
	#headLines = re.findall('<td align="left" class="bk11">(.+?)<br>(.+?)</td>',result,re.DOTALL)
	headLines = tree.xpath('//div[@id="ICRT_TOP_10"]//td[@align="left" and @class="bk11"]')
	#print clrTx('HEADLINES:','BLUE')
	ScreenI.append(clrTx('ICRT TOP 10 SONGS','BLUE'))
	cnt = 0
	for headLine in headLines:
		...
		if headLine.text is not None:
			if cnt % 2 == 0:			
				fixStr = headLine.text.replace(' ','+')
				LINKS.append('https://www.youtube.com/results?search_quert='+fixStr.lower())		
				ScreenI.append('    '+str(len(LINKS)-1).zfill(2)+'|'+clrTx(headLine.text,'YELLOW'))
			cnt+=1

	headLines = tree.xpath('//div[@id="US_TOP_10"]//td[@align="left" and @class="bk11"]')
	#print clrTx('HEADLINES:','BLUE')
	ScreenI.append(clrTx('US TOP 10 SONGS','BLUE'))
	cnt = 0
	for headLine in headLines:
		#print headLine		
		#print clrTx(headLine[1],'YELLOW')
	...
	headLines = tree.xpath('//div[@id="UK_TOP_10"]//td[@align="left" and @class="bk11"]')
	#print clrTx('HEADLINES:','BLUE')
	ScreenI.append(clrTx('UK TOP 10 SONGS','BLUE'))
	cnt = 0
	for headLine in headLines:
		#print headLine		
		#print clrTx(headLine[1],'YELLOW')
...
def setup_logging(level):
	global DB
	DB = logging.getLogger('get_icrt_top_song') #replace
	DB.setLevel(level)
...
def verify():
	global tTarget
	global args
	parser = argparse.ArgumentParser(description='A get_icrt_top_song Utility') #replace
	parser.add_argument('-v', '--verbose', dest='verbose', action = 'store_true', default=False, help='Verbose mode')
	parser.add_argument('query', nargs='*', default=None)
	parser.add_argument('-d', '--database', dest='database', action = 'store', default='/.hmDict/get_icrt_top_song.db') #replace
